...
type cmdHandler func(cli *FedisClient, args [][]byte) (interface{}, error)
...
type FedisClient struct {
	server *FedisServer
	db     *fedis.RedisDataStructure
}
...
func execClientCommand(conn redcon.Conn, cmd redcon.Command) {
	...
	client, _ := conn.Context().(*FedisClient)
	switch command {
	case "quit":
		_ = conn.Close()
	case "ping":
...
func set(cli *FedisClient, args [][]byte) (interface{}, error) {
	if len(args) != 2 {
...
func get(cli *FedisClient, args [][]byte) (interface{}, error) {
	if len(args) != 1 {
...
func hset(cli *FedisClient, args [][]byte) (interface{}, error) {
	if len(args) != 3 {
...
func sadd(cli *FedisClient, args [][]byte) (interface{}, error) {
	if len(args) != 2 {
...
func lpush(cli *FedisClient, args [][]byte) (interface{}, error) {
	if len(args) != 2 {
...
func zadd(cli *FedisClient, args [][]byte) (interface{}, error) {
	if len(args) != 3 {
